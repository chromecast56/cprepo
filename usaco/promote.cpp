...
Alternate solution, map the tree to its preorder traversal
Insert the nodes (values) in decreasing order, and query on the subtree (so in,in+sz(i)). Increment after querying
So in each value, it is either true or false that its value is greater than the current compared one, sum over the
 subarray.
