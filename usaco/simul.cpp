...
int simul() {
    int your_guess = rng()%3;
    int imposter = rng()%3;

    int rule_out = rng()%3;
    while(rule_out == imposter) rule_out = rng()%3;

    if (your_guess == rule_out) { //case 1
        return rng()%2;
    }
    else {
        //switch, so you dont want your initial guess to be the imposter
        return your_guess != imposter;
    }
}
int main()
{
