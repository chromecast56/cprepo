/*
            Fun data structure:
                ...
                Supports insertion of pairs, and supports queries of type: Given x, find the a_i with maximum y st. a_i.x >= x

                Uses the "solution bag" theory, where we maintain an invariant over the data structure.
                If a.x <= b.x and a.y <= b.y, then any query that would return a.y instead returns b.y -- b encapsulates a,
                which means that a should not be in the set

                If we maintain the invariant that all a st. a.x<=b.x && a.y<=b.y are deleted, we find that
                if a_1.x < a_2.x < a_3.x ..., then a_1.y > a_2.y > a_3.y ...   like a diagonal line
                Which means that the answer to a query x is simply the y of the closest a st. a.x >= x, since
                a.y is monotonically decreasing

                To maintain this, whenever we insert an {x,y}, we iterate on a_i <= x and continuously delete a
                that are encapsulated by {x,y} until invariant is achieved.
 */
