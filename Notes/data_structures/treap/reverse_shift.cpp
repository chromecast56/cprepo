...
/*
            ...
            - Reversal on subarray
            - Implicit indexing

            This allows it to perform the following functions:
            - Cyclic subarray shift queries (delete at end, reinsert at front)
            - Reverse queries
 */
...
private:
    ...
    ll search(pnode &t,int add,int key,bool b) { //0 = delete, 1 = get
        ...
        else res = search(t->l, key<=cur_key? add:1+cur_key,key,b);
        upd(t);
        return res;
    }
public:
