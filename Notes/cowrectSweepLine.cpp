...
int main()
{
    ...
    for (int h1=0;h1<N;h1++) //loop over all pairs of horz lines
        for (int h2=h1;h2<N;h2++)
        {
            ...
            for (int i=0;i<N;i++) //sweepline over all points
            {

                int itr = i;
                while(c[i].x==c[itr].x) //iterate over every 'x-group' together (so that G and H on same x are properly ignored)
                {
                    if (c[itr].y < cY[h1].y || c[itr].y > cY[h2].y) {
                        itr++;
                        continue;
                    }
                    if (!c[itr].h) { //there is a G, reset all values
                        sz=0,mx=MAX,Mx=0,my=MAX,My=0;
                        while(c[i].x==c[itr].x)     //make sure to finish looping itr to the end of the 'x-group'
                            itr++;
                        break;
                    }
