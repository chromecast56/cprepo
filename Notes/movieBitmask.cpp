...
int N,L;
int dp[1 << 20];
int D[20]; //create a bitmask of possible movies
vector<int> mov[20];
...
int main()
{
    ...
    dp[0] = 0;
    int res = 21;
    for (int msk = 0; msk < (1 << N); msk++) //loop all comb
    {
        if (dp[msk] == -1) continue;

        if (dp[msk] >= L) { //achieved goal
            int num = 0;
            for (int i = 0; i < N; i++)
                if (msk & (1 << i))
                    num++;
        ...
        for (int i=0;i<N;i++) //dp[msk] will affect all msks with one extra 1
        {
